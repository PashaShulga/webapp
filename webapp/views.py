...
@view_config(route_name='index', renderer='templates/index.pt', permission='view')
def index(request):
    try:
        mailer = request.registry['mailer']
        if request.unauthenticated_userid is not None:
            return {'login_status': 'Welcome, '+request.unauthenticated_userid, 'link': '/logout'}
        elif request.unauthenticated_userid is None:
            return {'login_status': 'login', 'link': '/login'}
        # print(mailer.__dict__)
        main = get_renderer('templates/main.pt')
    except DBAPIError:
...
def content(request):
    check_user = None
    print(request.unauthenticated_userid)
    if request.unauthenticated_userid is not None:
        ...
        if order.id is not None:
            check_user = DBSession.query(Content).all()[0]
